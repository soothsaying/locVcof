...
source("vc_SCAD.R")
source("vc_LASSO.R")
source("tunning_TLP.R")
source("tunning_SCAD.R")
source("tunning_LASSO.R")
source("tunning_AdLASSO.R")
source("vc.R")
library("splines")
library("mvtnorm")
...
###Test Data
########################################
n=500  ##############Change n=100, 250 or 500
p=2   ################ change p=1 or 3
rho=0.5
m=5
nt=n*m
N=ceiling(nt^{1/(2*p+3)})
...
mse_FULL<-matrix(0,nrep,d)
max_FULL<-matrix(0,nrep,d)
...
lambda_SCAD<-tunning_SCAD(x,tvar,y,N,p)
result<-vc_SCAD(x,tvar,y,xpred,tpred,N,p,lambda_SCAD,a)
alphahat_SCAD<-result[[1]]
...
if(sum(SCAD[,3])==0) {int_SCAD[r,]<-c(0,1)}
if(sum(SCAD[,3])>0)
{
...
###TLP
par_TLP<-tunning_TLP(x,tvar,y,N,p)
lambda<-par_TLP[[1]]
tau<-par_TLP[[2]]
result<-vc_TLP(x,tvar,y,xpred,tpred,N,p,lambda,tau)
alphahat_TLP<-result[[1]]
...
if(sum(TLP[,3])==0) {int_TLP[r,]<-c(0,1)}
if(sum(TLP[,3])>0)
{
...
###LASSO
weight=rep(1,d)
lambda_LASSO<-tunning_LASSO(x,tvar,y,N,p,weight)
result<-vc_LASSO(x,tvar,y,xpred,tpred,N,p,lambda_LASSO,weight)
alphahat_LASSO<-result[[1]]
...
if(sum(LASSO[,3])==0) {int_LASSO[r,]<-c(0,1)}
if(sum(LASSO[,3])>0)
{
...
###Adaptive LASSO
result<-vc(x,tvar,y,xpred,tpred,N,p)
alphahat_LSE<-result[[1]]
temp=apply(alphahat_LSE^2,2,mean)
ind=(1:d)[temp!=0]
weight<-rep(eps,d)
weight[ind]=1/temp[ind]
lambda_LASSO<-tunning_AdLASSO(x,tvar,y,N,p,weight)
result<-vc_LASSO(x,tvar,y,xpred,tpred,N,p,lambda_LASSO,weight)
alphahat_ADLASSO<-result[[1]]
...
if(sum(ADLASSO[,3])==0) {int_ADLASSO[r,]<-c(0,1)}
if(sum(ADLASSO[,3])>0)
{
